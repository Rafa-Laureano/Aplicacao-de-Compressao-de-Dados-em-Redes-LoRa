...
def monitor_resources(pid, stop_event, cpu_readings, mem_readings, sampling_interval=0.5):
    """Monitora continuamente o consumo de CPU e memória do processo específico"""
    try:
    ...
    while not stop_event.is_set():
        try:
            # CPU - precisa de pelo menos 0.1s para medição precisa
            cpu = process.cpu_percent(interval=sampling_interval)
            if cpu > 0:
...
def run_compression(process_command, num_repetitions):
    ...
    for _ in range(num_repetitions):
        ...
        # Inicia thread de monitoramento para o PID do processo gmix
        monitor_thread = threading.Thread(
            target=monitor_resources,
            args=(process.pid, stop_event, cpu_readings, mem_readings, 0.1)
        )
        ...
        stop_event.set()
        monitor_thread.join()
        stop_event.clear()  # Reseta para a próxima iteração
...
def main():
    ...
    output_csv = "consumo_diversificado_gmix.csv"
    num_repetitions = 100
    ...
    with open(output_csv, "w", newline='') as csvfile:
        ...
        for num_msg in range(1, 170):
            try:
                ...
                output_file = f"gmix_compressed_{num_msg}.bin"
                
                # Escreve arquivo temporário
                write_temp_file(data, temp_input_file)

                compress_cmd = ["./gmix", "-c", temp_input_file, output_file]
