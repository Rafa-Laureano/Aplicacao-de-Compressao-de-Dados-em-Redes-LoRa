...
void constroi_lista(unsigned short *t, lista *lista_ord){
	lista_ord->head=NULL;
	...
	for(int i = 0; i < TAB_OFFSET-1; i++){
		...
		if(t[i]){
			if(i < 10){
				insere_elemento_lista_ordenado(lista_ord,cria_no(t[i],48+i,NULL,NULL,NULL));
#ifndef GPS
			} else if(i >= 10 && i <= 35) {
				insere_elemento_lista_ordenado(lista_ord,cria_no(t[i],65+i-10,NULL,NULL,NULL));
#ifndef LOG
			} else if(i >= 36 && i <= 61) {
				insere_elemento_lista_ordenado(lista_ord,cria_no(t[i],97+i-36,NULL,NULL,NULL));
#endif
#endif
			} else if(i==ALFA){
				insere_elemento_lista_ordenado(lista_ord,cria_no(t[i],10,NULL,NULL,NULL));
#ifndef LOG				
			} else if(i==ALFA+1){
